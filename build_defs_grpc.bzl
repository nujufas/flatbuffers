...
def flatbuffer_cc_grpc_library(
        name,
        srcs,
        srcs_filegroup_name = "",
        outs = [],
    ...
    """
    output_headers = [
        (out_prefix + "%s.grpc.fb.h") % (s.replace(".fbs", "").split("/")[-1].split(":")[-1])
        for s in srcs
    ]
    output_sources = [
        (out_prefix + "%s.grpc.fb.cc") % (s.replace(".fbs", "").split("/")[-1].split(":")[-1])
        for s in srcs
    ]
    ...
    flatbuffer_library_public(
        name = srcs_lib,
        srcs = srcs,
        outs = outs + output_headers + output_sources,
        language_flag = "-c",
