...
"""
Rules for building Python flatbuffers with Bazel.
"""
...
def flatbuffer_py_library_public(
        name,
        srcs,
        outs,
        language_flag,
    ...
    Args:
      ...
      language_flag: Target language flag. One of [-p].
    ...
    genrule_cmd = " ".join([
        "echo \'Deubg: Starting FlatBuffer compilation\';",
        "echo OUTPREFIX: %s ;" % (out_prefix),
        "echo \'Debug: Source files: $(SRCS)\';",
        "echo \'Debug: Output files: $(OUTS)\';",
        "SRCS=($(SRCS));",
        "for f in $${SRCS[@]:0:%s}; do" % len(srcs),
            "echo PWD: $$PWD;",
            "echo CMD: %s $(location %s) " % (extra_env, flatc_path), " ".join(include_paths_cmd), " ".join(flatc_args), language_flag, output_directory, "$$f;",
            "%s $(location %s)" % (extra_env, flatc_path),
            ...
            "$$f;",
        "done",
        # "echo \'Debug: Finished FlatBuffer compilation\';",
    ])
    ...
    #         "echo 'Debug: Starting FlatBuffer reflection generation';",
    #         "SRCS=($(SRCS));",
    #         "for f in $${SRCS[@]:0:%s}; do" % len(srcs),
    #         "echo 'Debug: Generating reflection for $$f';",
    ...
    #         "$$f;",
    #         "done",
    #         "echo 'Debug: Finished FlatBuffer reflection generation';",
    #     ])
    #     reflection_outs = [
...
def flatbuffer_py_library(
    ...
    """A py_library with the generated reader/writers for the given flatbuffer definitions.
    ...
    Args:
      ...
      srcs_filegroup_name: Name of the output filegroup that holds srcs. Pass this
          ...
          flatbuffer_py_library that depends on this one's schemas.
      outs: Additional outputs expected to be generated by flatc.
      out_prefix: Prepend this path to the front of all generated files. Usually
          is a directory name.
      deps: Optional, list of other flatbuffer_py_library's to depend on. Cannot be specified
          alongside includes.
      includes: Optional, list of filegroups of schemas that the srcs depend on.
          Use of this is discouraged, and may be deprecated.
      include_paths: Optional, list of paths the includes files can be found in.
      py_include_paths: Optional, list of paths to add to the py_library includes attribute.
      flatc_args: Optional list of additional arguments to pass to flatc
          (e.g. --gen-mutable).
      visibility: The visibility of the generated py_library. By default, use the
    ...
    This produces:
      filegroup([name]_srcs): all generated .py files.
      filegroup(srcs_filegroup_name if specified, or [name]_includes if not):
          Other flatbuffer_py_library's can pass this in for their `includes`
          parameter, if they depend on the schemas in this library.
      Fileset([name]_reflection): (Optional) all generated reflection binaries.
      py_library([name]): library with sources and flatbuffers deps.
    """
    if deps and includes:
        fail("Cannot specify both deps and include in flatbuffer_py_library.")
    # if deps:
    #     includes = [d + "_includes" for d in deps]
    reflection_name = "%s_reflection" % name if gen_reflections else ""

    srcs_lib = "%s_srcs" % (name)
    flatbuffer_py_library_public(
        name = srcs_lib,
        srcs = srcs,
        outs = outs,
        language_flag = "-p --gen-all --gen-object-api",
    ...
    py_library(
        ...
        deps = [
            # "@flatbuffers//:flatbuffers_py"
        ] + deps,
        imports = py_include_paths,
        compatible_with = compatible_with,
        restricted_to = restricted_to,
        target_compatible_with = target_compatible_with,
